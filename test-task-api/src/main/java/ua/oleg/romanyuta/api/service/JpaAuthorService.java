...
public class JpaAuthorService implements AuthorService {
    ...
    private void saveRewards(Author author) {
        List<Reward> rewards = author.getRewards();
        if (rewards != null && !rewards.isEmpty()) {
            for (Reward reward : rewards) {
                saveReward(reward, author);
            }
        }
    }

    private void saveReward(Reward reward, Author author) {
        if (reward.getId() != null) {
            throw new BadRequestException("Unable to create new Author, rewards must not contain ids");
        }
